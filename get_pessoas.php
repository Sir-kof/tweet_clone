<?php 
	...
	$sql.= " ON (us.id_usuario = $id_usuario AND u.id = seguindo_id_usuario)";
	...
	if($resultado_id){
		while($registro = mysqli_fetch_array($resultado_id, MYSQLI_ASSOC)){
			echo "<a href='#' class='list-group-item'>";
				...
				echo "<p class='list-group-item-text pull-right'>";
					...
					$seguindo_usuario_sn = isset($registro['id_usuario_seguidor']) && 
					!empty($registro['id_usuario_seguidor']) ? 'S' : 'N';

					$btn_seguir_display = 'block';
					$btn_deixar_seguir_display = 'block';

					if($seguindo_usuario_sn == 'N'){
		...
		echo "erro na consulta de pessoas no banco de dados";
	}
